...
$dbexport = "faster__" . date("Y-m-d-H-i-s") . ".sql.gz";
...
try {
  ...
  // $fileUploadedID = uploadFileToDrive($access_token, $file_content, $mime_type);
  // echo "ID du fichier uploader: ". $fileUploadedID;
  // echo "Acces token generer ". $access_token;
} catch (Exception $e) {
...
function uploadFileToDrive($access_token, $file_content, $mime_type){
  $DRIVE_FILE_UPLOAD_URI = 'https://www.googleapis.com/upload/drive/v3/files';
  $apiURL = $DRIVE_FILE_UPLOAD_URI . '?uploadType=media'; 
  ...
  if ($http_code != 200) { 
      $error_msg = 'Echec lors de l\'upload vers Google Drive'; 
      if (curl_errno($ch)) { 
          $error_msg = curl_error($ch); 
      } 
      throw new Exception('Erreur '.$http_code.': '.$error_msg); 
  } 
